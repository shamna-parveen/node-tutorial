...
export default class AuthController {
  ...
  async login(req, res) {
    try {
      ...
      const user = await userRepo.getEmployeeByEmail(validatedRequest.email);
      ...
      if (user && (await user.matchPassword(validatedRequest.password))) {
        ...
        await loginDetailsRepo.addLoginDetails(
          user._id,
          user.email,
          true,
          token
        ...
        res.json({
          ...
          data: { user: await UserResponse.format(user), token: token },
        });
      } else {
